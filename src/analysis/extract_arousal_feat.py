...
def return_row_stats_df(time, type, demographic_dict, data, all_data=None, id=''):
    ...
    if len(data) == 0:
        ...
        if type == 'arousal':
            ...
            row_df['score'] = np.median(data['fusion'])
...
if __name__ == '__main__':
    ...
    for agg in agg_list:
        save_dict[agg] = dict()
        for loc in ['all', 'ns', 'pat', 'other']: save_dict[agg][loc] = pd.DataFrame()
    ...
    for nurse_id in nurse_id_list:
        ...
        gender = 'Male' if nurse_df.loc[nurse_df['participant_id'] == nurse_id]['gender'].values == 0 else 'Female'
        ...
        primary_unit = participant_info_df.loc[participant_info_df['ParticipantID'] == id]['PrimaryUnit'].values
        icu_str = 'Non-ICU'
